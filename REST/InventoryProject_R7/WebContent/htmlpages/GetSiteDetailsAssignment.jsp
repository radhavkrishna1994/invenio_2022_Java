								...
								<div class="art-post-body">
									<div class="art-post-inner art-article">
										...
										<div class="art-postcontent">
											...
											<form
												action="${pageContext.request.contextPath}/rest/inventory/assignSites"
												method="post">
												Select the Construction Head <select name="cHusername"
													class="s1">
													<c:forEach items="${chList}" var="ch">
														<option value="${ch.getUsername()}">${ch.getUsername()}</option>
													</c:forEach>
												</select>
												<table>
													<tr>
													...
													<c:forEach items="${siteList}" var="site">
														<tr>
															...
															<td><c:choose>
																	<c:when test="${site.getSiteInCharge()==null}">
																		<c:out value="Not Assigned" />
																	</c:when>
																	<c:otherwise>
																		<c:out value="${site.getSiteInCharge()}" />
																	</c:otherwise>
																</c:choose></td>
															<td><c:if test="${site.getConstructionHead()==null}">
																	<input type="checkbox" name="siteName"
																		value="${site.getSiteName()}" />
																</c:if> <c:if test="${site.getConstructionHead()!=null}">
																	<c:out value="Assigned" />
																</c:if></td>
															<td><c:if test="${site.getConstructionHead()!=null}">
																	<c:out value="${site.getConstructionHead()}" />
																</c:if> <c:if test="${site.getConstructionHead()==null}">
