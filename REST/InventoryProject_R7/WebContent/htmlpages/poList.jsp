								...
								<div class="art-post-inner art-article">
									...
									<div class="art-postcontent">
										...
										<%
											...
											%>
											<form action="${pageContext.request.contextPath}/rest/inventory/poListPO">
										<table border="1" align="center">
												...
												<%
												for (com.bean.PORequest x : poList) {
													session.setAttribute("PoForApproval",x);
											%>


<%-- ${pageContext.request.contextPath}/rest/inventory/updateRequirementStatus?requirementId=<%=x.getRequirementNo()%> --%>

												<tr>
													<td><%=x.getMaterial()%></td>
													<td><%=x.getTerm()%></td>
													<td><%=x.getQuantity()%></td>
													<td><%=x.getVendorName()%></td>
													<td><%=x.getPrice()%></td>
													<td><%= x.getRequestDate() %></td>
													<td><%= x.getSiteName() %></td>
													<td><span style="color:red;"><%=x.getPoStatus()%></span></td>
													<td>select <input type="checkbox" name="poMaterialList" value="<%=x.getPORNumber()%>"/></td>
